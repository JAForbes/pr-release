...
async function inferVersionExternal(options){
    let [owner,repo] = process.env.GITHUB_REPOSITORY.split('/')

    const lastRelease = await getLastRelease({ owner, repo })
    const version = await inferVersion({ owner, repo, lastRelease })
...
async function rollback({ version, ignore=null }){
    ...
    let revertSHA; {
        ...
        let newReleaseTag = semver.inc(currentReleaseTag, 'patch')
...
subcommands:
  ...
  ${magenta(`infer-version`)}
                        ...
                        Use the ${blue(`--quiet`)} option to log only the version.  Otherwise
