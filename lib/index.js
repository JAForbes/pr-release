...
async function pr(x){ 
    ...
    let lastRelease; {
        lastRelease = await octokit.rest.search.issuesAndPullRequests({
            q: `is:pr is:merged base:${target} repo:${owner}/${repo}`
            ,sort: 'updated'
            ,order: 'desc'
            ,per_page: 1
            ,page: 1
    ...
    let recentBranches; {
        ...
        recentBranches = 
            await octokit.paginate(octokit.rest.search.issuesAndPullRequests, {
                q: `is:pr is:merged target:${source} repo:${owner}/${repo}${mergedAfter}`
                ,sort: 'updated'
                ,order: 'desc'
            })
    }
    ...
    // let defaultTitle = `Release - ${await nextVersion()}`
    // let defaultBody = `
