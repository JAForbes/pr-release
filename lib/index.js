...
import { chalk, argv, $, fs, nothrow } from 'zx'
...
// If the target branch has diverged from the source we simply archive the old target
...
async function repairTarget(options){
    ...
    if( !options.force )  {
        error(`${target} is not an ancestor of ${source}, to repair ${target} re-run command with --force`)
    } else {
        let new_name = `${target}-archive-${Date.now()}`

        let sha = await getSha({ owner, repo }, source)

        await octokit.rest.repos.renameBranch({
            owner, repo, branch: target, new_name
        })
        ...
        let prs = 
        await octokit.paginate(octokit.rest.search.issuesAndPullRequests, {
            q: `is:pr is:open base:${new_name} repo:${owner}/${repo}`
        })

        for( let pr of prs ) {
            await octokit.rest.pulls.update({
                owner, repo, pull_number: pr.number, base: target
            })
        }
...
async function rollback({ version, ignore=null }){
    ...
    let ignoreFiles = []
    for( let glob of ignore ) {
        ignoreFiles.push( ... (await $`git ls-files ${glob}`).stdout.trim().split('\n'))
    }

    if(!version) {
        let x = await octokit.rest.repos.listReleases({
            owner, repo, per_page: 2, page: 1
        })
        x = x.data
        let [lastRelease, rollbackRelease] = x
    ...
    let currentTagSHA; {
        let x = await octokit.rest.git.getRef({
            owner, repo, ref: `tags/${currentReleaseTag}`
        })

        currentTagSHA = x.data.object.sha
    }
    ...
    let currentSlug =
        currentReleaseTag
            .replace(/./g, x => x.match(/[A-z]|[0-9]/) ? x : '_')
    
    let targetArchiveName = `${target}-rollback-${currentSlug}-${Date.now()}`
    ...
    let operations = []; {
        ...
        let changes = (await $`git diff --name-only c5012a1f1affc11147373305a37f932810d2337a 33b8f7670bf026ea84872f836f05102cb8e819f1 --diff-filter dr`).stdout.trim()
        changes = changes.split('\n')
        
        for( let x of changes ) {
    
            let y = await nothrow($`git show ${rollbackTagSHA}:${x}`)
            if( y.stderr.startsWith('fatal') && x.stderr.includes('does not exist') ) {
                let sha = await $`git show ${rollbackTagSHA}:${x} | git hash-object --stdin`
                sha = sha.stdout.trim()
                operations.push({
                    file: x
                    ,sha
        ...
        let deletions = (await $`git diff --name-only c5012a1f1affc11147373305a37f932810d2337a 33b8f7670bf026ea84872f836f05102cb8e819f1 --diff-filter D`).stdout.trim()
        
        for( let x of deletions ) {
            operations.push({
                file: x
                ,content: null
                ,type: 'blob'
        ...
        let renames = (await $`git diff --name-only c5012a1f1affc11147373305a37f932810d2337a 33b8f7670bf026ea84872f836f05102cb8e819f1 --diff-filter R`).stdout.trim()
        
        for( let x of renames ) {
            operations.push({
                file: x
                ,content: null
                ,type: 'blob'
...
/**
 * Rollbacks in prr are different to most other tools.
 * 
 * Instead of rolling back to a ref, we rollback to a github release.
 * And instead of generating a git revert, we do the opposite of a revert
 * we rewind time on the --source and --target branches and generate a PR
 * to reintroduce the changes targeting --source.
 * 
 * This way, parallel fixes and improvements can be merged as if the
 * rollback never happened.
 * 
 * In addition, you can pass a list of globs to find files that should
 * not be rolled back.  This is useful for files that contain the latest
 * version tag, or changelog.  As rolling back a version for a published
 * package, would not affect the external registry.
 * 
 * This behaviour is optional as you may automatically unpublish a package
 * in your rollback CI, if so, no files should be ignored.
 */
async function rollback2({ version, ignore=null }){
    let [owner,repo] = process.env.GITHUB_REPOSITORY.split('/')
    source; target;
    octokit;
    ...
    await octokit.rest.git.createRef({
        owner
        ,repo
        ,ref: `refs/heads/${targetArchiveName}`
        ,sha: targetSHA
    })

    // now that the branches have been backed up, lets rewind time
    ...
    await octokit.rest.git.updateRef({
        ...
        ,sha: rollbackTagSHA
        // ,sha: currentTagSHA
        // we just made a backup so its okay
        ,force: true
    })
    ...
    // now they are both rewound we can update either one
    // to include our ignored files, assuming there were ignore files

    if( ignoreFiles.length ) {

        // git show origin/main:package.json | git hash-object --stdin

        let changes = ignoreFiles
        changes = [...new Set(ignoreFiles)]

        await $`git fetch`
        changes = changes.map( async x => {
            let sha = (await $`git show origin/${targetArchiveName}:${x} | git hash-object --stdin`).stdout.trim()

            let verified = await $`git cat-file -p ${sha}`.exitCode == 0
            if(!verified){
                return Promise.reject(
                    'Could not obtain hash for file: ' + x
                )
            }
        ...
        let commit = await octokit.rest.git.createCommit({
            owner
            , repo
            , message: 'Ignored changes for rollback to '+version
                +`\n\n [skip ci]`
            , tree: tree.data.sha
            , parents: [base_tree]
        })

        // Now we have a new commit on --target, we need to fast forward --source
        // to have it as well
        await octokit.rest.git.updateRef({
            owner, repo, ref: `heads/${source}`, sha: commit.data.sha
        })
        await octokit.rest.git.updateRef({
            owner, repo, ref: `heads/${target}`, sha: commit.data.sha
        })
    }
    ...
    await octokit.rest.pulls.create({
        owner
        , repo
        , head: targetArchiveName
        , base: source
        , title: `🚨 Rolled back changes: ${version} to ${currentReleaseTag}`
        , body: `
