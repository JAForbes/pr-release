...
async function merge(options){
    ...
    // todo-james not ready to execute any of this yet
    process.exitCode = 1
    if( 1 == 1 ) { return; }

    // eslint-disable-next-line no-unused-vars
    let sha = process.env.GITHUB_SHA || await getSha(target)
    ...
    commit: if (options.commit){
        ...
        if( await fs.stat('package.json').catch( () => null ) ) {
            await $`npm version --no-verify --no-commit-hooks --no-git-tag-version ${version}`
        }
        ...
        await octokit.rest.git.updateRef({
            owner, repo, ref: target, sha: tree.sha
        })
    }
