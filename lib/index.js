...
async function rollback({ version, ignore=null }){
    ...
    let targetSHA = await getSha(target)
    let sourceSHA = await getSha(source)

    let currentSlug =
        currentReleaseTag
            .replace(/./g, x => x.match(/[A-z]|[0-9]/) ? x : '_')
    
    let targetArchiveName = `${target}-rollback-${currentSlug}`
    ...
    if( ignoreFiles.length ) {
        ...
        changes = changes.map( async x => {
            ...
            let failedVerification = await $`git cat-file -p ${sha}`.exitCode
        ...
        let commit = await octokit.rest.git.createCommit({
            owner
            , repo
            , message: 'Ignored changes for rollback to v'+version
                +`\n\n [skip ci]`
            , tree: tree.data.sha
            , parents: [base_tree]
        })
    ...
    await octokit.rest.pulls.create({
        ...
        These changes have been removed from ${source} and ${target}.  You can re-instate them by merging this PR.
        ...
        Because your ${source} and ${main} branches have been scrubbed clean of these changes you can continue to work on other unrelated change and release them
