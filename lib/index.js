...
import { URL } from 'url'
import { $, argv as _argv, chalk, fs } from 'zx'
...
let repoPath = (filepath) => path.resolve(__dirname, '..', filepath)
...
function groupBy(f, xs) {
	...
	for (let x of xs) {
		...
		key in idx ? idx[key].push(x) : idx[key] = [x]
	}
	return idx
}

async function getPackageJSONVersion({ owner, repo, ref , ref}) {
	let f
	try {
		f = await octokit.rest.repos.getContent(
			{ owner, repo, ref, path: 'package.json' },
		)
			.catch(() => null)
	...
	await debose(async () => {
		if (process.env.CI) {
			...
			if (!process.env.GITHUB_REF) {
				try {
					process.env.GITHUB_REF = (await $`git branch --show-current`).stdout
						.trim()
				} catch (e) {
			...
			if (response.status != 200) {
				throw new Error('Could not verify the validity of your GITHUB_TOKEN')
			}
...
export function getNextVersion({
	recentBranches,
	version,
	minimizeSemverChange = false,
}) {
	verbose('getNextVersion', { version, minimizeSemverChange })
	...
	let nextVersion
	{
		...
		if (semver.compare(nextVersion, version) == 0) {
			let level = prerelease ? ['prerelease', ...prerelease] : ['patch']
...
async function inferVersionExternal({ minimizeSemverChange = false } = {}) {
	...
	let recentBranches = await getRecentBranchesFromLastRelease({
		owner,
		repo,
		lastRelease,
	})
...
async function extractChangelog(options) {
	...
	let recentBranches = await getRecentBranchesFromPRCommitHistory({
		owner,
		repo,
		pull_number: lastRelease.number,
	})

	let lines = lastRelease ? lastRelease.body.split('\n') : []
	...
	let { version } = await inferVersion({ owner, repo })
	...
	let contributors = [...new Set(recentBranches.map((x) => x.user.login))]
	...
	if (options.out) {
		...
		await fs.writeFile(
			`${options.out}/changelog.md`,
			out.changelog,
		)

		await fs.writeFile(
			`${options.out}/patch.md`,
			out.patch,
		)

		await fs.writeFile(
			`${options.out}/minor.md`,
			out.minor,
		)

		await fs.writeFile(
			`${options.out}/major.md`,
			out.major,
		)
		...
		await fs.writeFile(
			`${options.out}/nextVersion`,
			out.nextVersion,
		)

		await fs.writeFile(
			`${options.out}/changeSummary.md`,
			out.changeSummary,
		)

		await fs.writeFile(
			`${options.out}/contributors.md`,
			out.contributorsBody,
		)

		await fs.writeFile(
			`${options.out}/contributors.md`,
			JSON.stringify(out.contributors),
		)

		await fs.writeFile(
			`${options.out}/labels.json`,
			JSON.stringify(out.labels),
		)

		await fs.writeFile(
			`${options.out}/pulls.json`,
			JSON.stringify(out.pulls),
		)
...
async function getRecentBranchesFromPRCommitHistory({
	...
	const pullCommits = await octokit.paginate(
...
async function getRecentBranchesFromLastRelease({ owner, repo, lastRelease }) {
	return getRecentBranchesFromLastCommitDate({
		owner,
		repo,
		mostRecentTagDate: lastRelease ? lastRelease.closed_at : null,
	})
}
...
async function getRecentBranchesFromLastCommitDate({
	...
	recentBranches = await octokit.paginate(
...
async function merge(options) {
	...
	const changelogOut = typeof options.changelog == 'string'
		? { out: options.changelog }
		: {}
	...
	let isAncestor =
		!await $`git fetch --all && git merge-base --is-ancestor ${`origin/${source}`} ${`origin/${target}`}`
			.exitCode

	if (!isAncestor && !options.force) {
		return error(
			`${source} is not an ancestor of ${target}.  Re-run with --force to enable a fast-forward release.`,
		)
	}
	...
	/* Find if we already have a tag for this version */ {
		...
		if (found) {
			info(
				'Version tag',
				nextVersion,
				'already exists, exiting with code zero.',
			)
			return null
		}
	}
	...
	commit: if (options.commit) {
		if (await fs.stat('package.json').catch(() => null)) {
			await $`npm version --no-verify --no-commit-hooks --no-git-tag-version ${nextVersion}`
				.exitCode
		}
		...
		if (isAncestor) {
			...
			verbose(
				'Attempting repair of target branch as it is not not an ancestor.',
			)
...
async function inferVersion({ owner, repo }) {
	let versionTypes = {
		...
		npm: await getPackageJSONVersion({ owner, repo, ref }),
		default: '0.0.0',
	}
...
function stripHTML(x) {
	return L.parseHTML(`<body><markdown>${x}</markdown></body>`).document
...
async function pr(options) {
	...
	let recentBranches = !lastRelease && mostRecentTagDate
	...
	function defaultBody() {
		...
		let defaultBody = (section_header +
			section_changelog +
...
async function diffChanges({ base, head, ignore = [] }) {
	await $`git fetch`
	let x = await $`git diff --raw ${base} ${head} ${
		ignore.map((x) => `:(exclude)${x}`)
	}`
	x = x.stdout
	// let example = `
	...
	let changes = x.flatMap((x) => {
		if (x.type == 'D') {
			...
			throw new Error(
				'Cannot rollback as changeset includes change type of ' + x.type,
			)
		}
	})
...
async function rollback({ version, ignore = null }) {
	...
	if (!version) {
		...
		x = x.data
		let [lastRelease, rollbackRelease] = x.sort((a, b) =>
			semver.compare(a.tag_name, b.tag_name) * -1
		)
		// find previous version
		rollbackRelease
		lastRelease
		if (!rollbackRelease) {
			return error('There is no previous release to rollback to.')
		}
		...
		if (!x) {
			return error('Could not find an existing release for version', version)
		}
...
async function featurePR() {
	...
	let changes = (await $`git fetch --all && git diff origin/next --name-only`)
		.stdout.trim().split('\n')
	...
	let useDraft
	{
		...
		useDraft = !repo2.private || plan.name == 'team'
	}

	let [existing] = await octokit.paginate(
		octokit.rest.search.issuesAndPullRequests,
		{
...
async function inferPreReleaseExternal({
	...
	try {
		...
		existingPreReleaseVersion = x
	} catch (e) {} /* No package published yet */

	let lastRelease = await getLastRelease({ owner, repo })

	let openBranches = lastRelease
		? await getRecentBranchesFromLastRelease({ owner, repo, lastRelease })
		: []
...
${magenta(`infer-prerelease`)}
    ...
    ${blue(`--publish=<tag>`)}
                        ...
                        ${
		blue(`--publish`)
	} will publish to the same release channel as preid (defaults to next)

                        ${
		blue(`--publish=alpha`)
	} will publish to the specified channel, in this case alpha
