...
async function merge(options){
    ...
    // PAT but not App token
    if ('permissions' in repoRes.data) {
        const p = repoRes.data.permissions
        if (!(p.admin || p.maintain)) {
            throw new Error('GH_TOKEN does not have sufficient permissions to create a release: ' + JSON.stringify(p))
        }
    }
    ...
    const releaseRes = await octokit.rest.repos.createRelease({
        owner
        ,repo
	...
	if (argv.prerelease) {
		try {
			// https://google.github.io/zx/quotes#assembling-commands
			$.verbose = true
			await $`${argv.prerelease.split(' ')}`
		} catch (e) {
            throw new Error('Could not execute prerelease script: '+ argv.prerelease)
		} finally {
			$.verbose = argv.verbose
		}
	}
...
async function pr(options){ 
    ...
    if ('permissions' in repoRes.data) {
        const p = repoRes.data.permissions
        if (!(p.admin || p.maintain)) {
            throw new Error('GH_TOKEN does not have sufficient permissions to create a release PR: ' + JSON.stringify(p))
        }
    }
    ...
    function updateTitle(thisRelease, version){
        return thisRelease.title.replace(new RegExp(escapeRegExp(oldVersion), 'g'), version)
    }
